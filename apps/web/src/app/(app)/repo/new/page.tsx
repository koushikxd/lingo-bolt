        ...
        <div className="flex w-full max-w-xs flex-col gap-2">
          {INDEXING_STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div
                className={`flex size-5 shrink-0 items-center justify-center rounded-full border text-[10px] transition-colors duration-200 ease-out ${i < step ? "border-primary bg-primary text-primary-foreground" : i === step ? "border-primary text-primary" : "border-border text-muted-foreground"}`}
              >
    ...
    <div className="motion-safe:animate-in motion-safe:fade-in mx-auto max-w-2xl space-y-6">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs tabular-nums text-muted-foreground">{filtered.length} repos</span>
        </div>
          ...
          <div className="max-h-112 space-y-1 overflow-y-auto">
            {visibleRepos.map((repo) => (
              <button
                ...
                className="flex w-full items-center justify-between rounded-md border border-border/50 p-3 text-left transition-colors duration-150 ease-out hover:bg-muted/50 active:scale-[0.99] disabled:opacity-50 motion-safe:transition-[background-color,transform] cursor-pointer"
              >
